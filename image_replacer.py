...
def main(args):
    ...
    try:
        ...
        # 如果UTF-8失败，尝试其他编码
        html_content = args.gray_html.read_text(encoding='latin-1')
    ...
    # 保存修改后的HTML（确保使用UTF-8编码）
    try:
        args.output_html.write_text(str(soup), encoding='utf-8')
        print("HTML文件已成功保存为UTF-8编码")
    except Exception as e:
        print(f"保存HTML文件时出错: {e}")
        # 备选方案：使用errors='ignore'忽略无法编码的字符
        args.output_html.write_text(str(soup), encoding='utf-8', errors='ignore')
        print("HTML文件已保存（无法编码的字符已被忽略）")
